...
<template>
  <div>
    ...
    <Teleport v-if="isMounted" :to="target" defer>
      ...
      <NuxtLink to="/conserje/eventos/crear">
        <Button size="sm">
    ...
    <EmptyState
      ...
      :description="filterStatus || filterUnitId ? 'Prueba cambiando los filtros' : 'Crea el primer evento'"
    >
      <template #action>
        <NuxtLink to="/conserje/eventos/crear">
          <Button>
            ...
            <TableRow v-for="item in events" :key="item.id">
              ...
              <TableCell class="text-right">
                <Button variant="ghost" size="icon" class="size-10" title="Ver" as-child>
                  <NuxtLink :to="`/conserje/eventos/${item.id}`">
                    <Eye class="size-4" />
                  </NuxtLink>
                </Button>
              </TableCell>
