...
export default defineEventHandler(async (event) => {
  ...
  // Deactivate all active passes for this staff
  const result = await db
    ...
    .returning({ id: serviceStaffPasses.id })

  if (result.length === 0) {
    throw createError({ statusCode: 404, message: 'No hay pase activo para revocar' })
  }

  return { data: { revoked: result.length } }
})
